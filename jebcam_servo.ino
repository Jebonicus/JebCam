...
float targetAngle = 0;  // default midpoint
float smoothedAngle = targetAngle;

const float SERVO_RANGE = 270;
...
void loop() {
  ...
  if ((currentMillis - lastServoUpdate) >= SERVO_UPDATE_INTERVAL) {
    ...
    int pulseWidth = round(((smoothedAngle+(SERVO_RANGE/2.0))/SERVO_RANGE) * (2400-480) + 480);//map(round(smoothedAngle), 0, 180, 544, 2400);
...
void onMqttMessage(int messageSize) {
  ...
  if (payload.length() > 0) {
    if (t == 1) {
      ...
      // Validate angle range (+-90 typical for servos)
      const float MAX_ANGLE=(SERVO_RANGE/2.0)+10;
