...
def draw_tracks(frame, tracks, targetTrack=None):
    # frame: HxWx3 BGR
    if len(tracks) > 0:
        print(f"draw_tracks: {len(tracks)} locked_track={targetTrack.id if targetTrack is not None else 'None'}")
    for t in tracks:
        x,y,w,h = t.get_state().astype(int)
        if t == targetTrack:
            trackBoxWidth = 3
            trackBoxColor = (0,0,255)
        else:
            trackBoxWidth = 1
            trackBoxColor = (0,255,0)
        cv2.rectangle(frame, (x,y), (x+int(w), y+int(h)), trackBoxColor, trackBoxWidth)
        cv2.putText(frame, f"ID:{t.id}, {t.label}", (x, y-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, trackBoxColor, 2)
...
def app_callback(identity_element, buffer):
    ...
    targetTrack = targetAcquisition.getTargetTrack()
    for t in tracks:
        if t == targetTrack:
            str=f"[LOCKED] "
        else:
...
def on_eos(bus, msg):
    global pipeline
    print("End of stream reached — restarting video...")
    pipeline.set_state(Gst.State.NULL)
    # Schedule restart after a short delay
    GLib.timeout_add(500, restart_pipeline)  # 500 ms delay

def stopstart_pipeline():
    global pipeline
    print("Stopping pipeline...")
    pipeline.set_state(Gst.State.NULL)
    GLib.timeout_add(500, restart_pipeline)  # 10s delay

def restart_pipeline():
    global pipeline
    print("Restarting pipeline...")
    pipeline.set_state(Gst.State.PLAYING)
    GLib.timeout_add(10000, stopstart_pipeline)  # 10s delay
